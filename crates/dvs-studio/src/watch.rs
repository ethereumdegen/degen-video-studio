...
/// What the watcher thread learned from one filesystem event.
enum Signal {
    /// Paths that were created, written, renamed or removed.
    Changed(Vec<PathBuf>),
    /// The backend lost events (kernel queue overflow). Nothing is known about *what*
    /// changed, so the only safe reading is "assume the document did".
    Lost,
}
...
fn pump(root: &Path, document: &Path, journal: &Path, sender: &mpsc::UnboundedSender<()>) {
    ...
    let Ok(mut watcher) = notify::recommended_watcher(move |event: notify::Result<notify::Event>| {
        let signal = match event {
            Ok(event) if changes_content(&event.kind) => Signal::Changed(event.paths),
            // Reads and metadata touches are dropped here rather than in `touches`, and
            // this is not a tidiness question. inotify reports `IN_ACCESS` for every read
            // of a watched file, and this window *reads* `project.json` on every reload.
            // Forwarding those closes a loop — reload, read, event, reload — that spins the
            // engine forever and, because an edit retires playback, makes the transport
            // stop about a second after every play with "the document changed".
            Ok(_) => return,
            // A watcher error means "something changed and I lost track of what": treat it
            // as a change, since a spurious reload is cheap and a missed one leaves a
            // permanently stale window.
            Err(_) => Signal::Lost,
        };
    ...
    loop {
        ...
        match incoming.recv() {
            Ok(Signal::Changed(paths)) if !touches(&paths, document, journal) => continue,
            Ok(_) => {}
            Err(_) => break,
        }
...
/// Whether an event kind means the bytes on disk are different now.
///
/// `Access` is every read. `Modify(Metadata(_))` is a permission or timestamp touch, which
/// includes the access-time update a read can trigger. Neither changes the document, and
/// both are generated by this process reading its own project. Everything else — create,
/// write, rename, remove, and the catch-all kinds a backend emits when it cannot be more
/// specific — is taken at face value.
...
mod tests {
    ...
    /// The bug this guards against was found by playing a real project in the real window:
    /// the transport stopped about a second into every play, saying "the document changed",
    /// while `project.json` had not been touched for two hours. inotify reports `IN_ACCESS`
    /// for reads, this window reloads the document on every wake-up, and the reload's own
    /// read produced the next wake-up — a loop that also retired playback each time round.
    #[test]
    fn reading_the_document_is_not_an_edit() {
        let dir = tempfile::tempdir().unwrap();
        let paths = project(dir.path());
        // A fresh project has no journal until its first op; create one so the read below
        // is the same pair of reads a reload performs.
        std::fs::write(paths.history(), b"{}\n").unwrap();
        let mut stream = Box::pin(watch(&paths));
        std::thread::sleep(Duration::from_millis(150));

        // Exactly what a reload does: read both files, repeatedly.
        for _ in 0..5 {
            let _ = std::fs::read(paths.project_json()).expect("read the document");
            let _ = std::fs::read(paths.history()).expect("read the journal");
        ...
        assert_eq!(
            collect(&mut stream, Duration::from_millis(400)),
            0,
            "reading the document must not look like an edit"
        );

        // …and the watcher is still live: a real write still wakes the window.
        std::fs::write(paths.history(), b"{}\n").unwrap();
        assert_eq!(
            collect(&mut stream, Duration::from_millis(600)),
            1,
            "filtering reads must not cost us writes"
        );
    }
